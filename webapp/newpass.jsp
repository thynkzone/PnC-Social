  ...
  <div class="verify-card">
    <% try {
      ...
      if (msg != null) {
        ...
        out.print("<h2><i class='fa-solid fa-key'></i> Password Reset</h2>");
        out.print("<div style='margin-bottom: 0.7rem; color: var(--text-secondary); text-align: center;'>Your new password has been set successfully</div>");
      }
    } catch(Exception ex) {}
    %>
    <a href='login.jsp' style="text-decoration: none; width: 100%;"><button class="button" style="width: 100%; max-width: 200px; margin: 1.2rem auto 0 auto;">Back to Login</button></a>
  </div>
</div>
	...
	try
	{
		...
		Object emkzObj = session.getAttribute("emk6");
        if (emkzObj == null) {
            session.setAttribute("msg", "Please Register first");
            response.sendRedirect("signup.jsp");
            return;
        }
        String emkz = emkzObj.toString();
		String emk7 = encdecry.encry(emkz, cf.word(), ss.word());
		...
		else if(request.getParameter("pwd")==null || request.getParameter("pwd").length() < 8 || !Pattern.matches("^(?=.*[A-Za-z])(?=.*\\d)(?=.*[@$!%*#?&+=_,.-/(){}])[A-Za-z\\d@$!%*#?&+=_,.-/(){}]{8,}$", request.getParameter("pwd"))) {
			try
		{
				response.sendRedirect("login.jsp");
				session.setAttribute("msg", "Password must be at least 8 charecters long!<br>(with at lesat one letter, one number and one special charecter)");
				return;
			}catch(Exception ex) {
			...
			try
		{
			...
			db.updatepass(enccrd, salt, peppere, emk7);
			
			out.print("Password successfully reset");
			out.print("<br>Please Log In with the email and new password from the login page");
			out.print("<br><br><p class = 'showBox'><a href='login.jsp'><button class='btn'>Log In</button></a></p>");
			session.removeAttribute("emk6");
		...
		System.out.print(ex+"at newpass.jsp");
		session.setAttribute("msg", "Please Register first");
		response.sendRedirect("signup.jsp");
	}
	%>
