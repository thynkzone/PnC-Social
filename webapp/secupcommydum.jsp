...
posteridcomm = antisamy.safe(request.getParameter("posteridcomm"));
postid = antisamy.safe(request.getParameter("postid"));
ntfno = antisamy.safe(request.getParameter("ntfno"));
commtext = base64.en(antisamy.safe(request.getParameter("commtext")));
posterid = antisamy.safe(request.getParameter("posterid"));

if (db.inputvalidatorString(commtext, 2)) {

if (posteridcomm != null && postid != null && commtext != null && posterid != null && ix.equals(posteridcomm)) {
	db.thyupdatecomm(ix, posteridcomm, postid, ntfno, commtext);
	session.setAttribute("msg", "Comment Updated Successfully!");
	response.sendRedirect("profile.jsp?userId="+posterid);
	return;
}
	...
	return;
				...
				response.sendRedirect("index.jsp");
				System.out.print(ex+"at secupcommydum.jsp");
				}
	%>
