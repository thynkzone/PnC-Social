...
if (db.verifycsrftoken(token)) {
			...
			String x = null;
			String y = null;
			x = antisamy.safe(request.getParameter("taxes"));
			y = antisamy.safe(request.getParameter("taxes2"));
			if (x != null && y != null) {
				db.thyrepdel(ix, x, y);
				response.sendRedirect("profile.jsp?userId="+y.substring(3));
				//if hia, above returns null as hide all deletes all posts and links to invlid postid in url bar
				return;
			}
	...
	return;
				...
				response.sendRedirect("index.jsp");
				System.out.print(ex+"at secpostrepdel.jsp");
				}
	%>
